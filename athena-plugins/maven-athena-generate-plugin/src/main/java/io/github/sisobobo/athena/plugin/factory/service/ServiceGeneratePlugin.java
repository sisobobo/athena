package io.github.sisobobo.athena.plugin.factory.service;
...
public class ServiceGeneratePlugin extends AbstractPlugin {
    ...
    protected void generate(MavenProject project, Db db, Condition condition, List<String> warnings) throws Exception {
        ...
        generateServiceImpl(models, project, overwrite);
    }

    private void generateServiceImpl(List<Model> models, MavenProject project, boolean overwrite) throws Exception {
        String packageName = project.getGroupId() + ".service";
        GenerateUtil.generateServiceImplFiles(models, project.getGroupId(), project.getBasedir().getPath(), packageName, overwrite);
    }
